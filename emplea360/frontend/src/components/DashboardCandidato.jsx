...
export default function DashboardCandidato() {
  ...
  // Limpieza de memoria para la URL de la foto de perfil
  useEffect(() => {
    return () => {
  ...
  // ==========================================
  // 🔥 3. FUNCIÓN DE PERSISTENCIA MANUAL/CONTROLADA EN LA NUBE
  // ==========================================
  const guardarDatosEnBaseDeDatos = async () => {
    ...
    try {
      ...
      if (respuesta.ok) {
        ...
        alert("No se pudieron guardar los datos en el servidor.");
      }
    } catch (error) {
      console.error("Error al persistir los datos manualmente:", error);
    } finally {
      setGuardando(false);
    }
  };
  ...
  const addConocimientoManual = () => {
    if (nuevoConocimiento && !conocimientos.includes(nuevoConocimiento)) {
      setConocimientos(prev => [...prev, nuevoConocimiento]);
  ...
  const handleAddEntrevistaSubmit = (e) => {
    e.preventDefault();
    setEntrevistas(prev => [...prev, { id: Date.now(), ...nuevaEntrevista, estado: 'Pendiente' }]);
    cerrarModal();
  };
    ...
    <div style={{ display: 'flex', height: '100vh', background: '#f8fafc', fontFamily: 'sans-serif', position: 'relative' }}>
      ...
      {tieneCambiosSinGuardar && (
        <div style={{
          ...
          <span style={{ fontSize: '14px' }}>⚠️ Tenés cambios sin sincronizar</span>
          <button 
            onClick={guardarDatosEnBaseDeDatos} 
            disabled={guardando}
            style={{
              background: '#22c55e',
              color: '#fff',
          ...
          >
            {guardando ? 'Guardando...' : 'Sincronizar Nubecita'}
          </button>
        </div>
      )}
